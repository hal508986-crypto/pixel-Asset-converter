...
| S-14 | 元絵の情報（画素数・可視色数・アスペクト比・見かけの格子・半透明率）をGUIに表示する |
| S-15 | 出力Canvasを元絵のアスペクト比に合わせる選択肢を用意する（長辺を指定する） |
| S-16 | 画面全体構図でアスペクト比が食い違うとき、歪むことをGUIで警告する |
| S-17 | 用途の表示名を実態へ合わせる（キャラクター→単体画像、キャラクターアニメーション→コマ割りアニメーション） |
| S-18 | 出力ディレクトリ名の `_b24` 固定を直し、実際のpalette上限を含める |
| S-19 | `outline_color` の握り潰しを直し、輪郭をGUIから選べるようにする |
| S-20 | GUIの出力Canvas上限を512から1280（64×20）へ引き上げる（N-2の解除） |
| S-21 | 大きいCanvasでドット確認画像がメモリを食い潰さないようにする |
...
| N-2 | ~~512×512以上のGUI提供~~ | **1.7で解除（S-20）。** 実写を背景アセットとして通す用途が出たため（[検証結果](../canvas_scale_study.md) 5.3節） |
...
- GUIのプリセット: 64×64 / 128×128（推奨） / 256×256 / 256×128 / 128×256 / 224×126 (16:9) /
  224×168 (4:3) / 512×512 / 1280×1280
- 自由入力: 幅・高さを個別指定。範囲は 16〜1280。既存の `animation_width/height` と同じ流儀。
- `resolve_character_gui_profile` は正方限定の検査をやめ、`min(width, height) >= 16` と
  `max(width, height) <= 1280` のみを検査する。`palette_budget=24` の既定は変えない。
...
`compiler_config_for_purpose` は `background_mode` と同じ `overrides.update()` で
`outline_color="off"` を強制しており、呼び出し側の指定が握り潰される。
S-8で背景を直したときに同種として残していたもの（N-8ではなく積み残し）。

`native_resolution` studyは `outline_color=config.outline` を渡しているが効いていない。
アニメーション経路は `CompilerConfig` を直接組むため、既に輪郭が効く。

#### 何が起きるか（実測、2026-09-10）

出力に見えている輪郭は**コンパイラが描いたものではなく、元絵の暗い縁が縮小されたもの**。
`outline_color` は既定 `off` なので、輪郭処理は動いていない。

| 出力 | 被写体bbox | 輪郭の暗帯 平均 | 中央値 |
|---|---|---|---|
| 128×128 | 76×73 | 2.22ドット | 1.0ドット |
| 256×256 | 153×147 | 3.85ドット | 2.0ドット |

元領域313pxを108px枠へ入れると0.34倍、216px枠なら0.69倍。同じ元輪郭が倍のドット数になる。
ドット絵は輪郭1ドットが基準のため、2ドットになると太く見える。

#### 契約

- `outline_color` を `setdefault` にし、呼び出し側の指定を尊重する。既定 `off` は変えない。
- GUIに「輪郭」（なし／黒／白）を出す。
- **`add_outline` はシルエットの外側に1ドット足す実装**であり、元絵の輪郭を置き換えない。
  元絵に輪郭がある素材で有効にすると輪郭は**太くなる**。
  この選択肢は「元絵を輪郭なしで描き、コンパイラに1ドット引かせる」ための道具である。
  GUIのツールチップでその前提を明示する。
- 輪郭を有効にすると量子化予算が1色分輪郭へ割かれる（既存実装）。

### 4.8 用途名と出力ディレクトリ名

背景を残す・画面全体構図・任意アスペクト比を入れた結果、「キャラクター」用途は
実際には**単体の1枚絵なら何でも通る**ものになった。実素材でも甲冑のクローズアップ、
青ベタ背景の飛行船、背景込みの海辺の風景がいずれもこの用途で通っている。
アニメーション側も同様で、背景込みの4コマ風景が通る。

表示名を実態へ合わせる（S-17）。内部の `purpose` 値（`character` / `character_animation` /
`terrain`）とCLIの `--purpose` は変えない。

| 内部値 | 旧表示名 | 新表示名 |
|---|---|---|
| `character` | キャラクター | 単体画像 |
| `character_animation` | キャラクターアニメーション | コマ割りアニメーション |
| `terrain` | 地形（64×64） | 地形（64×64）※変更なし |

#### 出力ディレクトリ名

`build_output_path` は単体・アニメーションの出力名に `_b24` をハードコードしていた。
palette上限を選べるようにした結果、**24色と48色が同じディレクトリへ出て上書きされる**。
地形は `{n}c` と実際の色数を含むため、この問題がない。

実際のpalette上限を含める形へ揃える（S-18）。

| 用途 | 旧 | 新 |
|---|---|---|
| 単体画像 | `character_{w}x{h}_b24` | `image_{w}x{h}_{n}c` |
| コマ割りアニメ | `character_animation_{w}x{h}_b24` | `animation_{w}x{h}_{n}c` |
| 地形 | `terrain_64x64_{mode}_{n}c_repeat-{on/off}` | 変更なし |

共有palette使用時の `_shared-{id}` 接尾辞は従来どおり付ける。
**既に書き出した成果物は旧名のまま残る。** 再コンパイルするまで新旧が混在する。

### 4.7 アスペクト比の扱い

画面全体構図（`pre_aligned`）は元絵をCanvasサイズへそのままリサイズする
（`nearest_pixelize` → `Image.resize`）。**アスペクト比が食い違うと非等倍になり、絵が歪む。**

実測（640×360の元絵に160×160の正方形マーカーを描いて計測、2026-09-10）:

| 出力Canvas | 構図 | マーカーの結果 |
|---|---|---|
| 128×128 | 画面全体 | 32×56 = **0.57:1 に潰れる** |
| 224×126（16:9） | 画面全体 | 56×56 = 1.00 維持 |
| 128×128 | 被写体を収める | 27×27 = 1.00 維持 |

被写体を収める構図は被写体bboxを等倍でフィットさせるため歪まない。歪むのは画面全体構図だけ。

対処は2つ。**リサイズ処理そのものは変えない**（既存出力の互換を壊すため）。

1. 出力Canvasを元絵のアスペクト比から決められるようにする（S-15）。
   長辺の画素数を利用者が指定し、短辺は `round(長辺 × 短辺 ÷ 長辺)` で求める。
   結果が16未満になる場合は16へ丸める。
2. 画面全体構図でアスペクト比が食い違うときに警告を出す（S-16）。
   判定は縦横比の差が5%を超えたとき。文言は4.4節。
...
### 4.10 上限1280（S-20・S-21）

**上限はGUIの方針であって、コアの制約ではない。** `CompilerConfig` は正の整数なら受け付ける
（`config.py` の `CanvasSpec`）。1.6までの512はN-2「用途が未定」という判断による据え置きで、
実写を背景アセットとして通す用途が出たため解除する（[検証結果](../canvas_scale_study.md) 5.3節）。

1280は 64×20。地形タイル64を20枚並べた幅にあたる。

#### 実測（6299×4199の写真、palette上限48、`pre_aligned`）
...
| 1024×683 | 4.4秒 | 504MB |
| 1280×853 | 5.0秒 | 504MB |
| 1280×1280 | 6.9秒 | 504MB |

コンパイル自体は問題ない。メモリは元絵（26M画素）の読み込みで決まり、出力サイズでは増えない。

#### ドット確認画像が上限を押し上げる（S-21）

倍率は `clamp(768 // 長辺, 4, 16)` なので、1280では下限に張り付いて4倍＝5120×5120になる
（[ドット確認画像の仕様](dot_preview_spec.md) 4.1節）。ここで**1.8GBを使う**（実測）。
原因は画像全体を `float64` へ変換していること（5120×5120×4×8 = 838MB を2枚分）。

倍率の契約は変えない。**変えるのは計算方法だけ**にする。

- 線の合成を、画像全体のfloat変換ではなく **256要素の対応表**（uint8→uint8）で行う。
  半透明の線色は固定なので、掛け算をやり直す必要がない。
- 市松模様の生成で `np.where` が `int64` の中間配列（5120×5120×3 = 630MB）を作るのをやめる。
- 見せ物用の暗くする処理も同じ対応表方式にする。

**出力は1画素も変えない。** 詳細と交点の扱いは[ドット確認画像の仕様](dot_preview_spec.md) 4.1節。
バイト一致は、全階調を踏む素材での回帰テストで固定する（少ない色数の素材では
丸めの違いが出ず、取りこぼす）。実測は1280×1280で 1.8GB/7.0秒 → 500MB/2.5秒。
分割とバジェット
...
| `test_character_gui_profile_rejects_out_of_range_canvas` | 16未満・1280超で `ValueError` |
| `test_main_window_exposes_extended_canvas_presets` | プリセットがGUIに並ぶ |
| `test_character_gui_profile_accepts_the_1280_ceiling` | 1280×1280が通り、1281で `ValueError`（S-20） |
| `test_main_window_canvas_inputs_reach_the_1280_ceiling` | 幅・高さ・長辺の入力上限が1280（S-20） |
| `test_main_window_exposes_the_large_canvas_presets` | 512×512と1280×1280がプリセットに並ぶ（S-20） |
| `test_inspect_preview_is_unchanged_by_the_lookup_rewrite` | 対応表方式でも出力がバイト一致（S-21） |
| `test_showcase_preview_is_unchanged_by_the_lookup_rewrite` | 同上（S-21） |
| `test_dot_previews_stay_within_a_memory_budget_for_a_large_canvas` | 大きいCanvasでピークメモリが上限以下（S-21） |
...
|---|---|
| 輪郭 | `なし` / `黒` / `白` |
| 輪郭のツールチップ | 「シルエットの外側に1ドットの輪郭を足します。元絵に輪郭が描かれている場合は二重になって太くなるため、輪郭なしで描いた元絵に使ってください」 |
| 出力Canvasサイズ プリセット | `64 × 64` / `128 × 128（推奨）` / `256 × 256` / `256 × 128` / `128 × 256` / `224 × 126（16:9）` / `224 × 168（4:3）` / `512 × 512` / `1280 × 1280` / `自由入力` |
| 自由入力の行 | ラベル「幅 × 高さ」、範囲16〜1280 |
...
| 元絵の情報 | `{幅}×{高さ} / {画素数}px / {アスペクト比}` と `可視{n}色 / {格子} / 半透明{n}%` |
| 元絵に合わせる選択肢 | `元絵に合わせる（長辺を指定）` |
| 長辺の入力 | ラベル「長辺」、範囲16〜1280 |
| 歪みの警告 | 「画面全体構図では元絵が出力Canvasへ引き伸ばされます。元絵は{a}、出力は{b}なので絵が歪みます」 |
...
| RC-03 | 256×256以上での所要時間が実用範囲か確認する（実測では512×512で4.6秒） | **完了**。実用範囲。1280×1280でも6.9秒（4.10節） |
...
| 1.4 | 2026-09-10 | 画面全体構図でアスペクト比が食い違うと絵が歪むことを実測で確認し、4.7節を追加。元絵の情報表示（S-14）、元絵に合わせるCanvas選択（S-15）、歪みの警告（S-16）を範囲に加えた |
| 1.5 | 2026-09-10 | 用途名を実態へ合わせ（S-17）、出力ディレクトリ名の `_b24` 固定による上書きを直す（S-18）。4.8節を追加 |
| 1.6 | 2026-09-10 | `outline_color` の握り潰しを直し、輪郭をGUIから選べるようにする（S-19・4.9節）。輪郭が外側に足される実装であることを明記 |
| 1.7 | 2026-09-10 | GUIの出力Canvas上限を512から1280（64×20）へ引き上げ、N-2を解除（S-20・4.10節）。実写を背景アセットとして通す用途が出たため。あわせて、大きいCanvasでドット確認画像が1.8GBを使う問題を対応表方式で解消する（S-21） |
